...
type BrandSearch = { dogrulanmis?: true; premium?: true };

export const Route = createFileRoute("/_site/markalar")({
  validateSearch: (s: Record<string, unknown>): BrandSearch => ({
    // "true" (JSON) veya "1" (elle yazılmış) kabul edilir; URL'de ?dogrulanmis=true olur.
    dogrulanmis: s.dogrulanmis === true || s.dogrulanmis === "1" ? true : undefined,
    premium: s.premium === true || s.premium === "1" ? true : undefined,
  }),
  // Filtre URL'de olduğu için SSR de aynı filtreyi uygulamalı; yoksa sunucu
  // filtresiz liste basar, istemci sonradan düzeltir (bot yanlış içerik görür).
  loaderDeps: ({ search }) => ({ dogrulanmis: search.dogrulanmis, premium: search.premium }),
  loader: async ({ deps }) => {
...
function BrandsPage() {
  ...
  async function load(p = page) {
    ...
    try {
      ...
      setBrands(r.items); setTotal(r.total);
    } finally {
      setLoading(false);
    }
  }
  useEffect(() => { fetchCategoriesWithCount().then((c) => setCats(c.map((x) => ({ slug: x.slug, name: x.name })))); }, []);
  useEffect(() => { load(page); /* eslint-disable-next-line */ }, [cat, sort, page, sp.dogrulanmis, sp.premium]);
  useEffect(() => { setPage(1); /* eslint-disable-next-line */ }, [cat, sort, sp.dogrulanmis, sp.premium]);
    ...
    <div>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10">
        <div className="bg-card rounded-2xl ring-1 ring-rule p-6 sm:p-8 mb-6">
          ...
          <form onSubmit={(e) => { e.preventDefault(); setPage(1); load(1); }} className="mt-6 flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-navy-mid" />
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Firma ara…" className="w-full h-11 rounded-full ring-1 ring-rule pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40" />
            </div>
            <select value={cat} onChange={(e) => setCat(e.target.value)} className="h-11 rounded-full ring-1 ring-rule px-4 text-sm bg-card">
              <option value="">Tüm kategoriler</option>
              {cats.map((c) => <option key={c.slug} value={c.slug}>{c.name}</option>)}
            </select>
            <select value={sort} onChange={(e) => setSort(e.target.value as SortKey)} className="h-11 rounded-full ring-1 ring-rule px-4 text-sm bg-card">
